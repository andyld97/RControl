...
{
    internal class Conrad8RelayCard
    { 
        /// <summary>
        /// The port
        /// </summary>
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="Conrad8RelayCard"/> class with default settings
        /// </summary>
        /// <param name="portName">Name of the serial port. "COM1", "/dev/ttyUSB0", "..." </param>
        public Conrad8RelayCard(string portName) :this (portName, 19200, 8)
        { }
        ...
        /// </summary>
        /// <param name="portName">Name of the serial port. "COM1", "/dev/ttyUSB0", "..." </param>
        /// <param name="baudRate">BaudRate of the serial port</param>
        /// <param name="dataBits">DataBits of the serial port</param>
        public Conrad8RelayCard(string portName, ushort baudRate, ushort dataBits)
        {
            this.port = new SerialPort(portName, baudRate, Parity.None, dataBits, StopBits.One);
        }
        ...
        /// <returns>card response frame</returns>
        public RelayCardStateResponse GetPortCommand(int cardAddress, int ports = 8)
        {
            var result = new RelayCardStateResponse
            {
                CardResponseFrame = SendCommandToCard(this.port,
                                                     (int)RequestCommand.GetPort,
                                                     (byte)cardAddress,
                                                     0)
            };

            result.RelayState = new CardRelayState(ports, result.CardResponseFrame.AddressByte);
        ...
        public RelayCardStateResponse ToggleCommand(int address, int portNumber, int ports = 8)
        {
            var toggleState = new CardRelayState(ports, address);
            toggleState.CardState[portNumber] = true;

            return this.ToggleCommand(toggleState, ports);
        }
        ...
        /// <param name="ports">Ports (=8)</param>
        /// <returns>relay card state</returns>
        public RelayCardStateResponse ToggleCommand(CardRelayState toggleState, int ports)
        {
            var result = new RelayCardStateResponse
            {
                CardResponseFrame = SendCommandToCard(this.port,
        ...
        /// <param name="ports">Ports (=8)</param>
        /// <returns>relay card state</returns>
        public RelayCardStateResponse SetSingleCommand(CardRelayState setSingleState, int ports)
        {
            var result = new RelayCardStateResponse
            {
                CardResponseFrame = SendCommandToCard(this.port,
        ...
        /// <param name="ports">Ports (=8)</param>
        /// <returns>relay card state</returns>
        public RelayCardStateResponse DelSingleCommand(CardRelayState delSingleState, int ports = 8)
        {
            var result = new RelayCardStateResponse
            {
                CardResponseFrame = SendCommandToCard(this.port,
